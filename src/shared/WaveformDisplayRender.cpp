...
namespace
{
using Cancel = QQDeBreathStemExport::Cancel;
...
bool render(const Request& request, Result& result, const QQDeBreathStemExport::Cancel& cancel)
{
    ...
    for (int i=0; i<regions.size(); ++i)
    {
        ...
        c.noize = r.type.equalsIgnoreCase("Noize");
    }
    for (int i=0; i<regions.size(); ++i)
    {
        if (cancel()) return false;
        ...
        const bool respondsToTarget = settings.normalizeBreath && peak > 1.0e-9f;
        ...
        if (!applyEq(contribution,rate,settings.globalEq,cancel) || !applyEq(contribution,rate,original.eqState,cancel)) return false;
        auto* dest=&result.scalable;
        // Zero-peak cores can have nonzero adjacent fades. They must not follow Norm Target.
        if (settings.normalizeBreath && !respondsToTarget)
        {
            if (result.fixed.getNumSamples()!=samples) { result.fixed.setSize(1,samples); result.fixed.clear(); }
            dest=&result.fixed;
        }
...
Worker::Worker() : Thread("QQDeBreath Waveform Preview") {}
