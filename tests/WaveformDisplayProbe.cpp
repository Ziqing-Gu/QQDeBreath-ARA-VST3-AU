...
public:
    ...
    static double compare(QQDeBreathWaveformEditor& current,LegacyWaveform124& old,int samples)
    {
    ...
    static QQDeBreathBridgeAnalysisResult analysis(int samples,int count)
    {
        ...
        for(int i=0;i<count;++i){QQDeBreathBridgeRegion region;region.type=i%4==0?"Noize":"Breath";
    ...
    {
        double worst=0;
        for(double rate:{44100.0,48000.0,96000.0})for(int mode=0;mode<8;++mode){
            ...
            QQDeBreathWaveformEditor current;LegacyWaveform124 old;
            ...
            const auto oldParams=[&]{LegacyWaveform124::DisplayProcessingParams q;q.enableFade=p.enableFade;q.normalizeBreath=p.normalizeBreath;q.fadeInMs=p.fadeInMs;q.fadeOutMs=p.fadeOutMs;q.breathTargetDb=p.breathTargetDb;q.breathGainDb=p.breathGainDb;q.breathEqState=p.breathEqState;return q;};
            ...
            for(int step=0;step<3;++step){p.breathGainDb=-10+step*7;p.breathTargetDb=-20+step*8;
                ...
                current.setMonitorState(mode%3!=0,mode%3!=1,mode%3!=2);old.setMonitorState(mode%3!=0,mode%3!=1,mode%3!=2);
        ...
        std::cout<<"24 waveform cases (3 scalar settings plus global/region EQ each); max relative display error="<<worst<<std::endl;
        {
